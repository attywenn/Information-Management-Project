...
function UserDashboard() {
    ...
    const [medicalCertificateMessage, setMedicalCertificateMessage] = useState("");
    ...
    const patientMedicalCertificateContext = useMemo(() => {
        if (user?.role !== "patient") {
            return { latestConsultation: null, hasAvailableCertificate: false };
        }

        const currentPatientCode = user?.patientCode || buildPatientCode(user?.username || "");
        const consultationsForPatient = consultations
            .filter((entry) => entry.patientCode === currentPatientCode)
            .sort((a, b) => {
                const aTime = new Date(a.completedAt || a.createdAt || 0).getTime();
                const bTime = new Date(b.completedAt || b.createdAt || 0).getTime();
        ...
        const latestConsultation = consultationsForPatient[0] || null;

        return {
            latestConsultation,
            hasAvailableCertificate: Boolean(latestConsultation?.diagnosis),
        };
    }, [consultations, user]);
    ...
    const handleSendMedicalCertificate = async () => {
        ...
        setMedicalCertificateMessage("");

        if (user?.role !== "patient") {
            setMedicalCertificateError("Only patients can request a medical certificate.");
            return;
        }

        if (!patientMedicalCertificateContext.hasAvailableCertificate) {
            setMedicalCertificateError("No completed consultation with a diagnosis was found yet.");
            return;
        }

        setMedicalCertificateLoading(true);
        try {
            const result = await sendMedicalCertificateToMyEmail({
                consultationId: patientMedicalCertificateContext.latestConsultation?.id || null,
            });
            setMedicalCertificateMessage(result?.message || "Medical certificate sent to your registered email.");
        } catch (error) {
                ...
                <section className="grid gap-4 grid-cols-1 xl:grid-cols-12">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm xl:col-span-4">
                        ...
                        <div className="mt-3 space-y-2">
                            ...
                            <button
                                type="button"
                                onClick={handleSendMedicalCertificate}
                                disabled={medicalCertificateLoading || !patientMedicalCertificateContext.hasAvailableCertificate}
                                className="w-full rounded-lg border border-brand-red bg-brand-red px-3 py-2 text-sm font-medium text-white hover:bg-brand-dark disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-200 disabled:text-slate-500"
                            >
                                {medicalCertificateLoading ? "Sending certificate..." : "Send Medical Certificate"}
                            </button>
                            <p className="text-xs text-slate-500">
                                Sends the latest completed consultation to your registered email as a password-protected PDF.
                            </p>
                            {medicalCertificateError ? (
                                <p className="text-xs font-medium text-red-600 bg-red-50 border border-red-100 rounded-lg p-2">
                                    {medicalCertificateError}
                                </p>
                            ) : medicalCertificateMessage ? (
                                <p className="text-xs font-medium text-green-700 bg-green-50 border border-green-100 rounded-lg p-2">
                                    {medicalCertificateMessage}
                                </p>
                            ) : null}
                        </div>
